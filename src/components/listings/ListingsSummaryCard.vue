    ...
    <q-card class="listings-summary-card">
      <q-card-section horizontal>
        <q-responsive class="col" :ratio="16 / 9" style="max-width: 100%">
          <q-carousel height="2rem" animated v-model="slideModel" arrows infinite>
            <q-carousel-slide
...
export default {
  ...
  computed: {
    carouselSlides() {
      ...
      const picsColl = this.currentListing.propPhotos || []
      picsColl.forEach(function (picObject, index) {
        let imageUrl = picObject.image
        if (imageUrl[0] === "/") {
          // Aug 2022 - need to retrieve images from BE server.
          // If I deploy from server where I can forward to BE
          // via caddy, will not need this.
          imageUrl = `${process.env.UPLOADS_BASE_URL}${imageUrl}`
        }
        carouselSlides.push({
          thumb: imageUrl,
          src: imageUrl,
    ...
    currentListingRoute() {
      const listingSlug = this.currentListing.id || "1"
